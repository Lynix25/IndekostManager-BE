...
public interface RoleRepository extends JpaRepository<Role, String> {
	...
	@Query(value = "SELECT * FROM role WHERE name LIKE :role_name AND is_deleted IS FALSE", nativeQuery = true)
	Role findByName(@Param("role_name") String roleName);
	
	@Query(value = "SELECT * FROM role WHERE name LIKE :role_name AND id NOT LIKE :role_id AND is_deleted IS FALSE", nativeQuery = true)
